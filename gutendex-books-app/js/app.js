...
function displayBooks(bookList) {
    ...
    bookList.forEach(book => {
        const bookElement = `
            <div class="book-card">
                <img src="${book.formats['image/jpeg'] || 'placeholder.jpg'}" alt="${book.title}" loading="lazy">
                <h3>${book.title}</h3>
...
async function loadBookDetails() {
    ...
    try {
        ...
        document.getElementById('book-title').innerText = book.title || 'Unknown Title';
        document.getElementById('book-author').innerText = `Author: ${book.authors[0]?.name || 'Unknown'}`;
        document.getElementById('book-genre').innerText = `Genre: ${book.subjects[0] || 'Unknown'}`;
        document.getElementById('book-cover').src = book.formats['image/jpeg'] || 'placeholder.jpg';
        document.getElementById('book-description').innerText = `Description: ${book.description || 'No description available.'}`;
    } catch (error) {
